...
Lemma d5_or_d6_Deq5: forall ret,
  d5_or_d6 ADeq5 ret.
Proof.
  intros.
  unfold d5_or_d6. intuition.
Qed.

Lemma d5_or_d6_Deq6: forall ret,
  d5_or_d6 (ADeq6 ret) ret.
Proof.
  intros.
  unfold d5_or_d6. intuition.
Qed.
...
Ltac solve_d5_or_d6 :=
  try apply d5_or_d6_Deq5;
  try apply d5_or_d6_Deq6.

Lemma inv_d10_d6: forall s s' acts pid ret,
  composed_lts.valid_execution_fragment (composed_array_queue L) s s' acts ->
...
Proof.
  ...
  - destruct (eq_nat_dec pid pid0).
    -- subst.
      ...
      rewrite Hbinds0 in H1;
      inversion H1; subst;
      unfold d5_or_d6 in Hv; intuition; try discriminate.
      simpl in H2. inversion H2; subst.
      erewrite <-IHvalid_execution_fragment; eauto;
      try (eapply substitute_eq_binds_v'; eauto);
      solve_d5_or_d6; simpl; auto.
      ...
      unfold binds in H3.
      rewrite H8 in H3.
      inversion H3; subst; solve_d5_or_d6.
      unfold binds in H3.
...
Lemma inv_xp_ref_le_vec_front_ref : forall k s,
  reachable_len' (composed_array_queue L) s k ->
  forall pid v (Hv : after_d6 v) (Hb : binds pid v (get_pc s)),
    snd ((get_xp s) pid) <= snd (get_vec_front H s pid).
Proof.
  induction k; intros.
  - unfold reachable_len' in H0.
    ...
    unfold get_xp, get_vec_front; simpl in *.
  ...
  - apply reachable_len_reconstruct in H0.
    ...
    -- inversion H3; subst; clear H3.
      ...
      inversion H1; subst; simpl in *.
      inversion H0; subst; simpl in *.
      unfold get_xp, get_vec_front in *; simpl in *.
      unfold get_ary, get_impl in *; simpl in *.
  ...
  destruct (Fin.eq_dec (Fin.of_nat_lt (mod_lt H (ArrayQueueImpl.front LState pid0)))
    (Fin.of_nat_lt (mod_lt H (ArrayQueueImpl.front LState pid)))).
    rewrite e.
    rewrite Vector.nth_replace_eq.
    simpl. rewrite H13.
    rewrite Htmp.
  ...
  -- inversion H3; subst; simpl in *; clear H3.
    ...
    --- subst.
      ...
      (* all : eapply substitute_eq_binds_v' in Hbinds0; eauto;
      ...
      rewrite Nat.eqb_refl.
      eapply inv_d6_ret_d5_internal_step with (v:=ADeq6 ret) (pid:=pid0) in Hreach; eauto.
      2 : {
      ...
      simpl in Hgather.
      pose proof Hvalid as HvalidTmp.
      eapply inv_d5_ret_d6_ret in Hvalid; eauto.
      2 : {
      ...
      apply inv_d5_instant with (k:=n) in Hstep; auto.
      simpl in Hstep.
      ...
      assert (Htmp : (Fin.of_nat_lt Hlt0) =
        (Fin.of_nat_lt (mod_lt H (ArrayQueueImpl.front (LState st2) pid0)))).
      apply Fin.of_nat_ext.
      rewrite Htmp.
Ltac solve_d5_or_d6 :=
  try apply d5_or_d6_Deq5;
  try apply d5_or_d6_Deq6.
      ...
      eapply inv_impl_front_keep' with (pid:=pid0) in HvalidTmp; eauto; simpl; auto; solve_d5_or_d6.
      unfold get_impl in *; simpl in *.
      rewrite HvalidTmp.
...
Ltac solve_after_d6 :=
  ...
  -- inversion H3; subst; clear H3.
    ...
    inversion Hb; subst;
    unfold after_d6 in Hv; intuition; subst; try discriminate;
    try (destruct H4; subst; discriminate);
    try (destruct H5; subst; discriminate).
    
    all : eapply IHk with (pid:=pid0) in Hreach; eauto; solve_after_d6;
...
Lemma inv_xp_ref_le_vec_front_ref_invariant:
  composed_lts.invariant (composed_array_queue L)
  (fun s => forall pid v (Hv : after_d6 v) (Hb : binds pid v (get_pc s)),
    snd ((get_xp s) pid) <= snd (get_vec_front H s pid)).
Proof.
  apply reachable_len'_to_invariant; eauto.
  intros.
  eapply inv_xp_ref_le_vec_front_ref; eauto.
Qed.

Lemma inv_vec_front_ref_not_decrease : forall s s' acts i
