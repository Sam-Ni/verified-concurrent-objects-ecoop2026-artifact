...
Definition after_d6 v :=
  v = ADeq10 \/
  v = ADeq11 \/
  (exists ret, v = ADeq12 ret) \/
  v = ADeq13 \/
  v = ADeq14 \/
  (exists ret, v = ADeq15 ret) \/
  v = ADeq26 \/
  v = ADeq27 \/
  v = ADeq28.

Lemma after_d6_Deq12: forall ret,
  after_d6 (ADeq12 ret).
Proof.
  intros.
  unfold after_d6.
...
Lemma after_d6_Deq15: forall ret,
  after_d6 (ADeq15 ret).
Proof.
  intros.
  unfold after_d6.
...
Lemma inv_d27_from_d6_ret : forall k (s : composed_lts.state (composed_array_queue L)),
...
Proof.
  ...
  -- inversion H4; subst. clear H4.
    ...
    unfold sync_aqimpl in H1.
      ...
      eapply IHk with (pid:=pid) in Hreach; eauto; solve_after_d6.
      destruct Hreach as [s' [acts [ret' [n' [Hvalid [Hb [Hgather [Hr Hlt]]]]]]]];
      exists s'.
      eexists.
      exists ret', n';
  ...
  -- inversion H4; subst. clear H4.
    ...
    unfold sync_aqimpl in H1.
      ...
      rewrite gather_pid_events_B_dist.
      simpl.
      assert (Htmp : pid <> pid0) by intuition.
      apply Nat.eqb_neq in Htmp; rewrite Htmp.
      rewrite app_nil_r.
      auto.
      simpl.
...
Lemma inv_d27_from_d6_ret_k_step : forall k (s : composed_lts.state (composed_array_queue L)),
  let sync_acc := s.(Compose.L1State) in
  let acc := sync_acc.(LState) in
  let sync_ary := acc.(Tensor.L1State) in
  let ary := sync_ary.(LState) in
...
Definition has_read_ary v pid pc res_ary ret :=
    (binds pid v pc /\
      binds pid (AryReadOk ret) res_ary /\ v = ADeq5) \/
    (binds pid v pc /\ v = ADeq6 ret).
...
Definition calculate_inv_d6_ret_d5_internal_step e :=
  match e with
  | ADeq5 => 0
  | ADeq6 r => 1
  | _ => 0
...
Lemma inv_d6_ret_d5_internal_step : forall k (s : composed_lts.state (composed_array_queue L)),
  let sync_acc := s.(Compose.L1State) in
  let acc := sync_acc.(LState) in
  let sync_ary := acc.(Tensor.L1State) in
  let ary := sync_ary.(LState) in
  ...
  forall pid ret v,
    ...
    exists s1 s2 acts n,
      ...
      (length (gather_pid_events_B pid acts)) = calculate_inv_d6_ret_d5_internal_step v /\
...
Proof.
  ...
  - apply reachable_len_reconstruct in H.
    ...
    -- inversion H3; subst.
      ...
      destruct (eq_nat_dec pid0 pid).
        ...
        eapply IHk with (pid:=pid) (ret:=ret) (v:=ADeq6 ret) in Hreach; eauto.
        destruct Hreach as [s1 [s2 [acts [n [Hvalid [Hst [Hb [Hgather [Hr Hlt']]]]]]]]].
        exists s1, s2, acts, n.
        intuition.
        eapply composed_lts.valid_execution_fragment_join' with (a':=[]); eauto.
  ...
  -- inversion H3; subst. clear H3.
    ...
    unfold sync_aqimpl in H0.
      ...
      eapply IHk with (pid:=pid0) (ret:=ret) (v:=ADeq5) in Hreach; eauto; simpl.
      destruct Hreach as [s1 [s2 [acts [n' [Hvalid [Hst [Hb [Hgather [Hr Hlt]]]]]]]]];
      exists s1, s2.
      eexists.
      exists n';
...
Lemma inv_d6_ret_d5_internal_step_k_step : forall k (s : composed_lts.state (composed_array_queue L)),
  let sync_acc := s.(Compose.L1State) in
  let acc := sync_acc.(LState) in
  let sync_ary := acc.(Tensor.L1State) in
  let ary := sync_ary.(LState) in
...
Lemma inv_d10_stuttering : forall s s' acts pid,
  composed_lts.valid_execution_fragment (composed_array_queue L) s s' acts ->
...
Lemma inv_d6_stuttering : forall s s' acts pid ret,
  composed_lts.valid_execution_fragment (composed_array_queue L) s s' acts ->
  binds pid (ADeq6 ret) (get_pc s) ->
  length (gather_pid_events_B pid acts) = 0 ->
  binds pid (ADeq6 ret) (get_pc s') \/
...
Proof.
  ...
  - destruct (eq_nat_dec pid0 pid).
    -- subst.
      ...
      inversion H4; subst; simpl in *.
      all : try (rewrite Hbinds0 in H1; discriminate).
      eapply inv_d10_stuttering in H0; eauto.
      unfold get_pc; simpl.
...
Lemma inv_d6_ret_d27_xp : forall (s : composed_lts.state (composed_array_queue L)) s',
  forall pid ret acts,
    (* reachable_len' (composed_array_queue L) s k -> *)
...
Proof.
  ...
  - intuition.
    ...
    destruct (eq_nat_dec pid pid0).
      ...
      eapply inv_d6_stuttering in H0; eauto.
  ...
  - destruct (eq_nat_dec pid pid0).
    subst.
      -- inversion H; subst; simpl in *.
        ...
        unfold after_d6 in H3; intuition; subst; try discriminate;
        try (destruct H; subst; discriminate);
        try (destruct H0; subst; discriminate);
        try (destruct H3; subst; discriminate);
        try (destruct H4; subst; discriminate);
...
Proof.
  ...
  - destruct (eq_nat_dec pid pid0).
    -- subst.
      ...
      inversion H2; subst.
      eapply inv_d6_stuttering with (ret:=ret) in H0; eauto.
      intuition.
...
Lemma inv_d5_instant : forall k s s' pid,
  reachable_len' (composed_array_queue L) s k ->
  binds pid (ADeq5) (get_pc s) ->
  composed_lts.step_L1 (composed_array_queue L)
  s pid ((@intL1 li_array li_cnt_cnt (array L) ArrayQueue.front_rear int_ary_read)) s' ->
  binds pid (AryRead ((get_impl s).(ArrayQueueImpl.front) pid mod L)) (get_ary s).(Array.requests L).
Proof.
  intros.
  ...
  apply valid_execution_fragment_com' in Hvalid.
    ...
    aa noB_preserves_AEnq5 Hvalid pid H0 acts Hgather.
    ...
    inversion H10; subst; simpl in *.
      ...
      eapply noB_preserves_ADeq5'' with (pid:=pid) in Hvalid; simpl; eauto.
      2 : {
        (eapply substitute_eq_binds_v'; eauto).
      }
      2 : {
