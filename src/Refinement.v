...
Lemma forward_simulation_ind_reconstruct: 
  forall (L1 L2 : lts li_null liB) f s1' s1 acts s2',
...
Definition composed_refines (L L' : composed_lts.composed_lts li_null liA liB) := 
  forall c_acts,
  composed_lts.in_traces L c_acts ->
  composed_lts.in_traces L' c_acts.

Lemma composed_refines_trans: forall (L : composed_lts.composed_lts li_null liA liB) L' L'',
  composed_refines L L' ->
  composed_refines L' L'' ->
  composed_refines L L''.
Proof.
  intros.
